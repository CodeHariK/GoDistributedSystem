...
func (h *Harness) CrashPeer(id int64) {
	tlog("Crash %d", id)
	h.DisconnectPeer(id)
	h.alive[id] = false
...
func (h *Harness) RestartPeer(id int64) {
	if h.alive[id] {
...
// are made.
func (h *Harness) PeerDropCallsAfterN(id int64, n int64) {
...
func (h *Harness) PeerDontDropCalls(id int64) {
...
// log index.
func (h *Harness) CheckCommitted(cmd int64) (nc int64, index int64) {
	h.t.Helper()
	h.mu.Lock()
	defer h.mu.Unlock()
...
// servers.
func (h *Harness) CheckCommittedN(cmd int64, n int64) {
	h.t.Helper()
...
func (h *Harness) CheckNotCommitted(cmd int64) {
	h.t.Helper()
	h.mu.Lock()
	defer h.mu.Unlock()
	...
	for i := int64(0); i < h.n; i++ {
		if h.connected[i] {
			for c := 0; c < len(h.commits[i]); c++ {
				...
				if gotCmd == cmd {
					h.t.Errorf("found %d at commits[%d][%d], expected none. h.commits[i]:%+v", cmd, i, c, h.commits[i])
				}
			}
		}
	}
...
func (h *Harness) SubmitToServer(serverId int64, cmd int64) int64 {
